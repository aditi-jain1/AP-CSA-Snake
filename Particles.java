...
public class Particles extends Rectangle{
	...
	private static Particles[] shimmerEffect = new Particles[numParticles];
	private static int dxeee;
	private static int dyeee;
	private static int particleSize=2;

	public Particles(int x, int y, Color c, int dx, int dy, int size) {
		super(x,y,size, size);
	}
	...
	public static Particles[] makeParticles() {
		for (int i=0; i<numParticles; i++) {
			...
			shimmerEffect[i] = new Particles(dxeee,dyeee, Color.white, dxeee, dyeee, randomInt(1,4));
			particleSize = 5;
		}
	...
	public static void moveParticles() {
		counter++;
 		for (int i=0; i<numParticles; i++) {
 			if (i<numParticles/8) {
 				shimmerEffect[i] = new Particles(shimmerEffect[i].x,shimmerEffect[i].y+speed, Color.white, 1,1,particleSize);
 			}
 			else if (i<2*(numParticles/8)) {
 				shimmerEffect[i] = new Particles(shimmerEffect[i].x+speed,shimmerEffect[i].y, Color.white, 1,1,particleSize);
 			}
 			else if (i<3*(numParticles/8)) {
 				shimmerEffect[i] = new Particles(shimmerEffect[i].x-speed,shimmerEffect[i].y, Color.white, 1,1,particleSize);
 			}
 			else if (i<4*(numParticles/8)) {
 				shimmerEffect[i] = new Particles(shimmerEffect[i].x-speed,shimmerEffect[i].y-speed, Color.white, 1,1,particleSize);
 			}
	...
	public static void moveParticles2() {
		counter++;
 		for (int i=0; i<numParticles; i++) {
 			...
 			shimmerEffect[i] = new Particles(shimmerEffect[i].x,shimmerEffect[i].y+3, Color.white, 1,1,particleSize);
 		}
 		/*
 		if (counter%8==0) {
 			speed = -speed;
	...
	public static Particles[] makeParticles2(int brickX, int brickY, Color col) {
		for (int i=0; i<numParticles; i++) {
			dissolveParticlesCounter++;
			dxeee = randomInt(brickX-10,brickX+50);
			dyeee = randomInt(brickY-10,brickY+50);
			shimmerEffect[i] = new Particles(dxeee,dyeee, col, dxeee, dyeee,2);
		}
		return shimmerEffect;
	}
